...
![Laravel 13.16](https://img.shields.io/badge/Laravel-13.16-FF2D20?logo=laravel&logoColor=white)
![Livewire 4.3](https://img.shields.io/badge/Livewire-4.3-4E56A6?logo=livewire&logoColor=white)
...
- **Emails en cola** (los cuatro Mailables implementan `ShouldQueue`):
  ...
  - al cliente: cita rechazada, con el motivo que escribes en Telegram.
...
- **Panel de administración** (`/panel`): contraseña única compartida (sin tabla de usuarios), máximo 5 intentos de login por minuto e IP. Lista paginada de citas (30 por página) con acciones de **confirmar** (email + evento en Google) y **cancelar** (borra el evento de Google). También gestiona los **días bloqueados**, y no deja bloquear un día que ya tiene citas activas.
...
- **Bilingüe ES / EN**: el idioma de la interfaz se elige con el selector y se guarda en una cookie (`/lang/{locale}`). Si no hay cookie se usa el idioma por defecto. Cada cita guarda el idioma del cliente para sus emails.
...
Versiones exactas del `composer.lock` y del `package-lock.json`:
...
| laravel/pint (dev) | 1.29.3 |
| phpunit/phpunit (dev) | 12.5.30 |
| laravel/pail (dev) | 1.2.7 |
| tailwindcss / @tailwindcss/vite | 4.3.1 |
| vite | 8.0.16 |
| laravel-vite-plugin | 3.1.0 |
| concurrently (dev) | 9.2.1 |
...
- **Composer 2**.
- **Node.js 20.19+ o 22.12+** y npm (lo pide Vite 8) para compilar el CSS y el JS.
...
Sin `npm run build` las páginas dan error 500, porque falta el manifiesto de Vite.
...
| `APP_LOCALE` | Idioma de Laravel. El de la interfaz de citas sale de `config/appointments.php` (`default_locale`). | `es` |
...
| `composer test` / `php artisan test` | Tests con PHPUnit. |
| `./vendor/bin/pint` | Formato del código PHP (preset de Laravel). |
| `npm run dev` / `npm run build` | Vite en desarrollo / compilación para producción. |
...
tests/                   tests de ejemplo del esqueleto
```
...
2. `composer install --no-dev --optimize-autoloader`, `npm ci` y `npm run build`.
...
- **Color de acento**: `resources/css/app.css`, sección **RE-THEME** (marcada con ⭐). Por ejemplo, azul: `--accent: #2563eb; --accent-glow: #3b82f6;`. Luego, `npm run build`.
- **Textos**: `lang/es/*.php` y `lang/en/*.php`.
- **Página de inicio**: `resources/views/welcome.blade.php` es una demo; cámbiala por la tuya.

### Limitaciones conocidas

- **Tests**: solo están los dos tests de ejemplo del esqueleto de Laravel. La lógica de reservas no tiene tests automatizados.
...
- **Cancelar desde el panel** no envía email al cliente ni pide motivo; solo borra el evento de Google, que manda su propio aviso de cancelación a los invitados. El email con motivo solo sale al rechazar desde Telegram.
- **Eventos de Google**: se consultan los próximos 16 días naturales. Con `APPOINTMENTS_DAYS_AHEAD` alto, los últimos días que se ofrecen pueden no tener en cuenta tu calendario.
...
- **Queued emails** (all four Mailables implement `ShouldQueue`):
  ...
  - to the client: booking rejected, with the reason you typed in Telegram.
...
- **Admin panel** (`/panel`): a single shared password (no users table), with at most 5 login attempts per minute per IP. Paginated booking list (30 per page) with **confirm** (email + Google event) and **cancel** (deletes the Google event) actions. It also manages **blocked days** and won't block a day that already has active bookings.
...
- **Bilingual ES / EN**: the UI language is picked with the switcher and stored in a cookie (`/lang/{locale}`). Without the cookie, the default language is used. Each booking stores the client's language for its emails.
...
Exact versions from `composer.lock` and `package-lock.json`:
...
- **Composer 2**.
- **Node.js 20.19+ or 22.12+** and npm (required by Vite 8) to build the CSS and JS.
...
Without `npm run build`, pages return a 500 error because the Vite manifest is missing.
...
| `APP_LOCALE` | Laravel's locale. The booking UI language comes from `config/appointments.php` (`default_locale`). | `es` |
...
| `composer test` / `php artisan test` | PHPUnit tests. |
...
2. Run `composer install --no-dev --optimize-autoloader`, `npm ci` and `npm run build`.
...
- **Accent color**: `resources/css/app.css`, **RE-THEME** section (marked with ⭐). For example, blue: `--accent: #2563eb; --accent-glow: #3b82f6;`. Then run `npm run build`.
- **Copy**: `lang/es/*.php` and `lang/en/*.php`.
- **Home page**: `resources/views/welcome.blade.php` is a demo; replace it with your own.

### Known limitations

- **Tests**: only Laravel's two skeleton example tests are included. The booking logic has no automated tests.
...
- **Cancelling from the panel** doesn't email the client or ask for a reason; it only deletes the Google event, and Google sends its own cancellation notice to the invitees. The email with a reason is only sent when rejecting from Telegram.
- **Google events**: only the next 16 calendar days are checked. With a large `APPOINTMENTS_DAYS_AHEAD`, the last offered days may ignore your calendar.
