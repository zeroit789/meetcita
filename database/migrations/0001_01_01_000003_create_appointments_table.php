...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('appointments', function (Blueprint $table) {
            ...
            // ES: Datos de contacto del cliente. EN: Client contact data.
            $table->string('name');                  // ES: nombre (obligatorio) · EN: client name (required)
            $table->string('email');                 // ES: email (obligatorio) · EN: client email (required)
            $table->string('phone')->nullable();     // ES: teléfono (opcional) · EN: phone (optional)

            // ES: Fecha y hora de la cita. EN: Date and time of the booking.
            $table->date('date');                    // ES: día de la cita · EN: booking day (YYYY-MM-DD)
            $table->string('time', 5);               // ES: hora del slot "HH:MM" · EN: slot "HH:MM"
            ...
            $table->string('google_event_id')->nullable();  // ES: id del evento · EN: event id
            $table->string('google_meet_url')->nullable();   // ES: enlace Meet (solo online) · EN: Meet link (online only)
        ...
        if ($driver === 'sqlite') {
            ...
            // ES: Índice único parcial de PostgreSQL. EN: PostgreSQL partial unique index.
            DB::statement('CREATE UNIQUE INDEX appointments_active_slot_unique ON appointments (date, time) WHERE status <> \'cancelada\'');
        } else {
